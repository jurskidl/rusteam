...
fn gamma_5_ideal(t: f64, p: f64) -> f64 {
    ...
    for x in 0..REGION_5_COEFFS_IDEAL_JI.len() {
        let ji: i32 = REGION_5_COEFFS_IDEAL_JI[x];
        let ni: f64 = REGION_5_COEFFS_IDEAL_NI[x];
        sum += ni * tau.powi(ji);
    }
    sum + pi.ln()
}
...
fn gamma_5_res(t: f64, p: f64) -> f64 {
    ...
    for x in 0..REGION_5_COEFFS_RES_II.len() {
        let ii: i32 = REGION_5_COEFFS_RES_II[x];
        let ji: i32 = REGION_5_COEFFS_RES_JI[x];
        let ni: f64 = REGION_5_COEFFS_RES_NI[x];
